...
class Protopool_cap(nn.Module):
    def __init__(self, num_prototypes: int, num_descriptive: int, num_classes: int,
        ...
        # Determine number of channels output by the backbone's last conv/batchnorm
        conv_modules = [m for m in self.features.modules() if isinstance(m, nn.Conv2d)]
        bn_modules = [m for m in self.features.modules() if isinstance(m, nn.BatchNorm2d)]
        if len(conv_modules) > 0:
            first_add_on_layer_in_channels = conv_modules[-1].out_channels
        elif len(bn_modules) > 0:
            first_add_on_layer_in_channels = bn_modules[-1].num_features
        else:
            raise Exception('Could not determine feature extractor output channels')
