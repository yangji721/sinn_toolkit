...
from concept_models.tesnet.util.helpers import list_of_distances


def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
                   coefs=None, log=print, k=7):
    '''
    ...
    for i, (image, label) in enumerate(dataloader):
        ...
        with grad_req:
            ...
            output, min_distances, _, capfactor = model(input)
            del input
            # compute loss
            cross_entropy = torch.nn.functional.cross_entropy(output, target)

            if class_specific:
                cap_loss = torch.linalg.norm(model.module.cap_width_l2)
                ...
                inverted_distances, _ = torch.topk((max_dist - min_distances) * prototypes_of_correct_class, k ,dim=1)
            ...
            if class_specific:
                if coefs is not None:
                    loss = (coefs['crs_ent'] * cross_entropy
                          ...
                          +coefs['cap_coef']*cap_loss)
                else:
                    loss = cross_entropy + 0.8 * cluster_cost - 0.08 * separation_cost + 1e-4 * l1 + 1 * orth_cost - 1e-7 * subspace_sep + 3e-3*cap_loss
    ...
    log('\tp cap loss: \t{0}'.format(cap_loss.item()))
...
def train(model, dataloader, optimizer, class_specific=False, coefs=None, log=print, topk = 7):
    ...
    return _train_or_test(model=model, dataloader=dataloader, optimizer=optimizer,
                          class_specific=class_specific, coefs=coefs, log=log, k = topk)


def test(model, dataloader, class_specific=False, log=print, topk = 7):
    log('\ttest')
    model.eval()
    return _train_or_test(model=model, dataloader=dataloader, optimizer=None,
                          class_specific=class_specific, log=log, k = topk)
