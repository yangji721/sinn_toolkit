...
class ClassificationLitModel(pl.LightningModule):
    def __init__(self, dataset, base_network, experiment_name):
        ...
        self.train_acc3 = torchmetrics.Accuracy(
            task="multiclass", top_k=3, num_classes=num_classes, compute_on_cpu=True
        )
        ...
        self.eval_acc3 = torchmetrics.Accuracy(
            task="multiclass", top_k=3, num_classes=num_classes, compute_on_cpu=True
        )
        ...
        self.eval_f1 = torchmetrics.F1Score(
            task="multiclass", num_classes=num_classes, average="macro", compute_on_cpu=True
        )
        ...
        self.eval_acc3_ema = self.eval_acc3.clone() if has_ema else None
        self.eval_acc5_ema = self.eval_acc5.clone() if has_ema else None
        self.eval_f1_ema = self.eval_f1.clone() if has_ema else None
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            self.train_acc3(outputs, labels)
    ...
    def eval_step(self, batch, _batch_idx, val_or_test):
        ...
        self.log(f"{val_or_test}_acc3", self.eval_acc3, on_epoch=True, prog_bar=False)
        self.log(f"{val_or_test}_acc5", self.eval_acc5, on_epoch=True, prog_bar=False)
