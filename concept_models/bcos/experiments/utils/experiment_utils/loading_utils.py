...
def device_safe_load_state_dict_from_path(path: PathLike) -> StateDictType:
    ...
    # 尝试使用weights_only=True加载（PyTorch 2.6+的安全模式）
    try:
        state_dict = torch.load(path, map_location="cpu", weights_only=True)
    except Exception as e:
        # 如果失败，回退到旧模式（weights_only=False）
        print(f"Warning: Failed to load with weights_only=True, falling back to weights_only=False. Error: {e}")
