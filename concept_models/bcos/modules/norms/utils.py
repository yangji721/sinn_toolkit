...
def _create_name_with_suffix(old_name, suffix):
    """Static function to create name with suffix that can be pickled."""
    return old_name + suffix


def _append_to_name(mod, suffix):
    old_name = mod.__class__.__name__
    # Use a lambda that calls the static function
    mod._get_name = lambda: _create_name_with_suffix(old_name, suffix)
